...
const cvFileField = yup
  ...
  .test("fileType", "Please upload a PDF or Word document.", (value) => {
    ...
    return allowed.includes(value.type) || /\.(pdf|doc|docx)$/i.test(value.name);
  })
  .test("fileSize", "CV must be 5MB or smaller.", (value) => {
    if (!(value instanceof File)) return false;
    return value.size <= 5 * 1024 * 1024;
  });

export const recruitmentApplySchema = yup.object({
  firstName: nameField.label("First name"),
  lastName: nameField.label("Last name"),
  email: emailField,
